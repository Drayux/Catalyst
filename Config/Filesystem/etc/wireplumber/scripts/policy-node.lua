-- OVERRIDE OF /usr/share/wireplumber/scripts/policy-node.lua
-- MODIFICATIONS:
...
-- Specify a target node via the target.node property
-- Chains directly into findDefinedTarget (within handleLinkable)
...
function lookupCustomRoute (properties)
  ...
  -- Determine to which virtual device our node should connect
  local device = nil
	for i, arr in pairs (custom_routes) do
	...
	if ( device == nil ) then
		Log.debug ("No rule found for application '" .. tostring (bin) .. "'")
    return false
	end

  -- Find the node ID of the device
  local device_node = linkables_om:lookup {
    Constraint { "node.name", "=", device }
  }
  if ( device_node == nil or device_node.properties["node.id"] == nil ) then
		Log.warning ("No device found with node.name '" .. tostring (device) .. "'")
    return false
	end

  -- Update the si_node's properties
  local target_node = device_node.properties["node.id"]
  properties["node.target"] = target_node
...
function findDefinedTarget (properties)
  ...
  -- This appears to be relevant to handling reconnections
  if metadata then
  ...
  -- Some nodes may be configured to not connect anywhere
  if target_value == "-1" then
    return nil, false, node_defined
  end

  -- Quick lookup of target node
  ...
  -- Full lookup if the predicted key leaves nothing
...
function handleLinkable (si)
  ...
  -- attempt to define a target based upon our routing rules (non-stock configuration)
  -- matches_rule currently unused
  local matches_rule = lookupCustomRoute(si_props)
