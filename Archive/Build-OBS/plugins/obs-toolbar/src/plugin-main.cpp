...
{
	return obs_module_text("Basic.Title");
}
...
// toolbarDock::toolbarDock(QWidget *parent) : QDockWidget(parent, Qt::Window)
toolbarDock::toolbarDock(QWidget *parent) : QDockWidget(parent), ui(new Ui::toolbarDock)
{
	cpuUsage = os_cpu_usage_info_start();

	ui->setupUi(this);
	titlebar = this->titleBarWidget();
	this->setWindowTitle(obs_module_text("Basic.Title"));
...
/*void toolbarDock::checkStreamingStatus(bool inTransition, bool transitionState)
{
	...
	char* tooltext = obs_module_text("Basic.Main.StartStreaming");
	char* statusText = obs_module_text("Basic.Stats.Status.Inactive");

	if (!inTransition) {
		if (isStreaming) {
			tooltext = obs_module_text("Basic.Main.StopStreaming");
		} else {
		...
		if (transitionState) {
			tooltext = obs_module_text("Basic.Main.Connecting");
			statusText = obs_module_text("Basic.Main.Connecting");
		} else {
			tooltext = obs_module_text("Basic.Main.StoppingStreaming");
			statusText = obs_module_text("Basic.Main.StoppingStreaming");
		}
